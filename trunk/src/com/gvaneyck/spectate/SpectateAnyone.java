    ...
    public static void setupClient() {
        ...
        if (conf.exists()) {
            try {
                ...
                JOptionPane.showMessageDialog(f, "Encountered an error when parsing config.txt", "Error", JOptionPane.ERROR_MESSAGE);
            }
        }
        ...
        if (lol == null || air == null) {
            JOptionPane.showMessageDialog(f, "The installation location does not appear to be valid, make sure lollocation is correct in config.txt.", "Error", JOptionPane.ERROR_MESSAGE);
            System.exit(0);
        }
    ...
    public static synchronized void handleSpectate() {
        ...
        try {
            ...
            if (result.get("result").equals("_error")) {
                ...
                // If there's more than a 5s wait, create a thread to
